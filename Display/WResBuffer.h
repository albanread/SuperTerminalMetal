...
// SuperTerminal v2
//
// WRES (Wide Resolution) pixel buffer for 432×240 graphics with 256-color palette.
// Wide version of XRES designed for 16:9 displays.
//
// THREAD SAFETY:
...
#ifndef SUPERTERMINAL_WRES_BUFFER_H
#define SUPERTERMINAL_WRES_BUFFER_H
...
/// WResBuffer: 432×240 pixel buffer with 256-color palette (Wide Mode)
///
/// Responsibilities:
/// - Store pixels at 432×240 resolution (16:9 aspect ratio, square pixels)
...
///
/// Resolution:
/// - WRES: 432×240 pixels (103,680 pixels)
/// - Memory: 103,680 bytes (101 KB per buffer)
/// - Total for 4 buffers: 414,720 bytes (405 KB)
///
...
public:
    ...
    WResBuffer(const WResBuffer&) = delete;
    WResBuffer& operator=(const WResBuffer&) = delete;
    WResBuffer(WResBuffer&&) = delete;
    WResBuffer& operator=(WResBuffer&&) = delete;
    ...
    /// @param dst_y Destination Y coordinate
    /// @note Thread Safety: Safe to call from any thread
    void blitFrom(const WResBuffer* src, int src_x, int src_y, int width, int height,
                  int dst_x, int dst_y);
    ...
    void blitFromTransparent(const WResBuffer* src, int src_x, int src_y, int width, int height,
                             int dst_x, int dst_y);
    
    /// Get raw pixel data for rendering
    /// @return Pointer to pixel buffer (103,680 bytes)
    /// @note Thread Safety: Caller must hold lock via getMutex()
    /// @note Data format: uint8_t[432*240] in row-major order
    const uint8_t* getPixelData() const { return m_pixels.get(); }
    
    /// Get buffer dimensions
    /// @param width Output width (always 432)
...
private:
    /// Pixel storage: 432×240 uint8_t array
    /// Each pixel stores 8-bit palette index (0-255)
    /// Memory: 103,680 bytes (101 KB)
...
#endif // SUPERTERMINAL_WRES_BUFFER_H