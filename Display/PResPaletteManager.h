//
// PResPaletteManager.h
// SuperTerminal v2
//
// Manages hybrid 256-color palette for PRES mode (1280×720 graphics).
...
#ifndef SUPERTERMINAL_PRES_PALETTE_MANAGER_H
#define SUPERTERMINAL_PRES_PALETTE_MANAGER_H
...
/// Preset palette types for PRES
enum class PResPalettePreset {
...
/// PResPaletteManager: Manages hybrid palette for PRES mode (1280×720)
///
/// Responsibilities:
...
///
/// Palette Structure:
/// - **Indices 0-15:** Per-row palette (720 rows × 16 colors = 11,520 entries)
/// - **Indices 16-255:** Global palette (240 colors, shared across all rows)
/// - **Total unique colors:** 11,760 palette entries
///
/// Memory Layout (CPU):
/// - Per-row: 720 rows × 16 colors × 4 bytes (RGBA) = 46,080 bytes
/// - Global: 240 colors × 4 bytes (RGBA) = 960 bytes
/// - Total CPU storage: 47,040 bytes
///
/// GPU Layout (float4):
/// - Per-row: 720 rows × 16 colors × 16 bytes (float4) = 184,320 bytes
/// - Global: 240 colors × 16 bytes (float4) = 3,840 bytes
/// - Total GPU buffer: 188,160 bytes
///
/// Usage:
...
public:
    ...
    static constexpr int ROW_COUNT = 720;         // 720 scanlines
    static constexpr int TOTAL_INDICES = 256;     // 0-255

    /// Create a new PResPaletteManager
    ...
    PResPaletteManager(const PResPaletteManager&) = delete;
    PResPaletteManager& operator=(const PResPaletteManager&) = delete;
    PResPaletteManager(PResPaletteManager&&) = delete;
    PResPaletteManager& operator=(PResPaletteManager&&) = delete;

    /// Set a per-row palette color (indices 0-15 only)
    /// @param row Row/scanline number (0-719)
    ...
    void loadPresetPalette(PResPalettePreset preset);

    /// Load preset palette to specific row range
    /// @param preset Preset type (only 16-color presets valid)
    /// @param startRow First row to modify (0-719)
    /// @param endRow Last row to modify (0-719, inclusive)
    /// @note Thread Safety: Safe to call from any thread
    /// @note Only affects indices 0-15 for specified rows
    void loadPresetPaletteRows(PResPalettePreset preset, int startRow, int endRow);
    ...
    /// @return Pointer to float palette buffer (188,160 bytes)
    /// @note Thread Safety: Caller must hold lock via getMutex()
    /// @note Data format: float4[720][16] (per-row) + float4[240] (global)
    ...
    /// @return Size of GPU palette buffer (188,160 bytes)
...
private:
    /// Per-row palette storage: 720 rows × 16 colors × 4 components (RGBA)
    /// Total: 46,080 bytes
    uint8_t m_perRowPalettes[720][16][4];
    ...
    /// Total: 188,160 bytes (720×16×16 + 240×16)
...
#endif // SUPERTERMINAL_PRES_PALETTE_MANAGER_H
