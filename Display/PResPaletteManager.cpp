//
// PResPaletteManager.cpp
// SuperTerminal v2
//
// Implementation of hybrid 256-color palette manager for PRES mode (1280×720).
// 720 lines with their own initial 6 colors, 256 per line. Ten thousand colors available
// 0 transparent 1..15 per line palette. 16.. screen palette.
...
PResPaletteManager::PResPaletteManager()
    : m_floatData(nullptr)
...
void PResPaletteManager::initDefaultPalette() {
...
void PResPaletteManager::setPerRowColor(int row, int index, uint32_t rgba) {
...
uint32_t PResPaletteManager::getPerRowColor(int row, int index) const {
    std::lock_guard<std::mutex> lock(m_mutex);
...
void PResPaletteManager::setGlobalColor(int index, uint32_t rgba) {
    std::lock_guard<std::mutex> lock(m_mutex);
...
uint32_t PResPaletteManager::getGlobalColor(int index) const {
    std::lock_guard<std::mutex> lock(m_mutex);
...
void PResPaletteManager::setAllRowsToColor(int index, uint32_t rgba) {
    std::lock_guard<std::mutex> lock(m_mutex);
...
void PResPaletteManager::loadPresetPalette(PResPalettePreset preset) {
    std::lock_guard<std::mutex> lock(m_mutex);

    switch (preset) {
        case PResPalettePreset::IBM_RGBI:
        ...
        case PResPalettePreset::GRAYSCALE: {
        ...
        case PResPalettePreset::RGB_CUBE_6x8x5: {
...
void PResPaletteManager::loadPresetPaletteRows(PResPalettePreset preset, int startRow, int endRow) {
    std::lock_guard<std::mutex> lock(m_mutex);
...
const float* PResPaletteManager::getPaletteDataFloat() const {
    std::lock_guard<std::mutex> lock(m_mutex);
...
bool PResPaletteManager::isDirty() const {
    std::lock_guard<std::mutex> lock(m_mutex);
    return m_dirty;
}

void PResPaletteManager::clearDirty() {
    std::lock_guard<std::mutex> lock(m_mutex);
    m_dirty = false;
}

void PResPaletteManager::convertToFloat() const {
    // Must be called with mutex held

    float* dst = m_floatData.get();

    // Convert per-row palettes (720 rows × 16 colors × 4 floats)
    for (int row = 0; row < ROW_COUNT; row++) {
...
void PResPaletteManager::unpackRGBA(uint32_t rgba, uint8_t& r, uint8_t& g, uint8_t& b, uint8_t& a) {
    a = (rgba >> 24) & 0xFF;
    r = (rgba >> 16) & 0xFF;
    g = (rgba >> 8) & 0xFF;
    b = rgba & 0xFF;
...
uint32_t PResPaletteManager::packRGBA(uint8_t r, uint8_t g, uint8_t b, uint8_t a) {
...
void PResPaletteManager::enableGradientAutomation(int paletteIndex, int startRow, int endRow,
...
void PResPaletteManager::enableBarsAutomation(int paletteIndex, int startRow, int endRow,
...
void PResPaletteManager::disableAutomation() {
    std::lock_guard<std::mutex> lock(m_mutex);
    m_gradientAutomation.enabled = false;
    m_barsAutomation.enabled = false;
}

void PResPaletteManager::updateAutomation(float deltaTime) {
    std::lock_guard<std::mutex> lock(m_mutex);
...
bool PResPaletteManager::isAutomationActive() const {
    std::lock_guard<std::mutex> lock(m_mutex);
    return m_gradientAutomation.enabled || m_barsAutomation.enabled;
}

void PResPaletteManager::applyGradientAutomation() {
    // Must be called with mutex held
...
void PResPaletteManager::applyBarsAutomation() {
    // Must be called with mutex held
