//
// XResPaletteManager.cpp
// SuperTerminal v2
//
// Implementation of hybrid 256-color palette manager for XRES mode.
//
...
XResPaletteManager::XResPaletteManager()
    : m_floatData(nullptr)
...
void XResPaletteManager::initDefaultPalette() {
...
void XResPaletteManager::setPerRowColor(int row, int index, uint32_t rgba) {
...
uint32_t XResPaletteManager::getPerRowColor(int row, int index) const {
    std::lock_guard<std::mutex> lock(m_mutex);
...
void XResPaletteManager::setGlobalColor(int index, uint32_t rgba) {
    std::lock_guard<std::mutex> lock(m_mutex);
...
uint32_t XResPaletteManager::getGlobalColor(int index) const {
    std::lock_guard<std::mutex> lock(m_mutex);
...
void XResPaletteManager::setAllRowsToColor(int index, uint32_t rgba) {
    std::lock_guard<std::mutex> lock(m_mutex);
...
void XResPaletteManager::loadPresetPalette(XResPalettePreset preset) {
    std::lock_guard<std::mutex> lock(m_mutex);
    
    switch (preset) {
        case XResPalettePreset::IBM_RGBI:
        ...
        case XResPalettePreset::GRAYSCALE: {
        ...
        case XResPalettePreset::RGB_CUBE_6x8x5: {
...
void XResPaletteManager::loadPresetPaletteRows(XResPalettePreset preset, int startRow, int endRow) {
    std::lock_guard<std::mutex> lock(m_mutex);
...
const float* XResPaletteManager::getPaletteDataFloat() const {
    std::lock_guard<std::mutex> lock(m_mutex);
...
bool XResPaletteManager::isDirty() const {
    std::lock_guard<std::mutex> lock(m_mutex);
    return m_dirty;
}

void XResPaletteManager::clearDirty() {
    std::lock_guard<std::mutex> lock(m_mutex);
    m_dirty = false;
}

void XResPaletteManager::convertToFloat() const {
...
void XResPaletteManager::unpackRGBA(uint32_t rgba, uint8_t& r, uint8_t& g, uint8_t& b, uint8_t& a) {
    a = (rgba >> 24) & 0xFF;
    r = (rgba >> 16) & 0xFF;
    g = (rgba >> 8) & 0xFF;
    b = rgba & 0xFF;
...
uint32_t XResPaletteManager::packRGBA(uint8_t r, uint8_t g, uint8_t b, uint8_t a) {
...
void XResPaletteManager::enableGradientAutomation(int paletteIndex, int startRow, int endRow,
...
void XResPaletteManager::enableBarsAutomation(int paletteIndex, int startRow, int endRow,
...
void XResPaletteManager::disableAutomation() {
    std::lock_guard<std::mutex> lock(m_mutex);
    m_gradientAutomation.enabled = false;
    m_barsAutomation.enabled = false;
}

void XResPaletteManager::updateAutomation(float deltaTime) {
    std::lock_guard<std::mutex> lock(m_mutex);
...
bool XResPaletteManager::isAutomationActive() const {
    std::lock_guard<std::mutex> lock(m_mutex);
    return m_gradientAutomation.enabled || m_barsAutomation.enabled;
}

void XResPaletteManager::applyGradientAutomation() {
    // Must be called with mutex held
...
void XResPaletteManager::applyBarsAutomation() {
    // Must be called with mutex held
