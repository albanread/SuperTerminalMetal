...
#include <memory>
extern std::unique_ptr<SuperTerminal::VoiceController> g_voiceController;
#endif
...
#ifdef VOICE_CONTROLLER_ENABLED
    // Terminal mode - call g_voiceController directly
    // TODO: Implement terminal version
#else
    // GUI mode - use AudioManager
        g_voiceController->setWaveform(voiceNum, static_cast<VoiceWaveform>(waveform));
#endif
    }
#else
    ST_LOCK;
...
#else
    // GUI mode - use AudioManager
        g_voiceController->setFrequency(voiceNum, frequencyHz);
#endif
    }
#else
    ST_LOCK;
...
#else
    // GUI mode - use AudioManager
        g_voiceController->setNote(voiceNum, midiNote);
#endif
    }
#else
    ST_LOCK;
...
#ifdef VOICE_CONTROLLER_ENABLED
    if (g_voiceController) {
        g_voiceController->setNoteName(voiceNum, noteName);
    }
#else
    ST_LOCK;
    ST_CHECK_PTR(ST_CONTEXT.audio(), "AudioManager");
    ST_CONTEXT.audio()->voiceSetNoteName(voiceNum, noteName);
#endif
}
...
#else
    // GUI mode - use AudioManager
        g_voiceController->setEnvelope(voiceNum, attackMs, decayMs, sustainLevel, releaseMs);
#endif
    }
#else
    ST_LOCK;
...
#else
    // GUI mode - use AudioManager
        g_voiceController->setGate(voiceNum, gateOn != 0);
#endif
    }
#else
    ST_LOCK;
...
#else
    // GUI mode - use AudioManager
        g_voiceController->setVolume(voiceNum, volume);
#endif
    }
#else
    ST_LOCK;
...
#else
    // GUI mode - use AudioManager
        g_voiceController->setPulseWidth(voiceNum, pulseWidth);
#endif
    }
#else
    ST_LOCK;
...
ST_API void st_voice_direct(const char* destination) {
    ...
    if (destination && strlen(destination) > 0) {
        // Enable render-to-file mode
        ST_CONTEXT.audio()->voiceSetRenderMode(true, destination);
    } else {
        // Disable render mode, back to live playback
...
ST_API uint32_t st_voice_direct_slot(int slotNum, float volume, float duration) {
    ...
    // Render voice controller audio to memory and save to sound slot
    uint32_t soundId = ST_CONTEXT.audio()->voiceRenderToSlot(slotNum, volume, duration);
...
#else
    // GUI mode - use AudioManager
        printf("[st_voice_direct_slot] Successfully rendered to slot %d with ID %u (volume: %.2f, duration: %.1fs)\n", 
               slotNum, soundId, volume, duration);
    } else {
        ST_SET_ERROR("Failed to render voice audio to sound slot");
#endif
    }
