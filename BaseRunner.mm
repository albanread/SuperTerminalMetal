...
- (void)setupMenuBar {
    ...
    NSMenuItem* formatItem = [[NSMenuItem alloc] initWithTitle:@"Format Code (BASIC)"
    ...
    NSMenuItem* renumberItem = [[NSMenuItem alloc] initWithTitle:@"Renumber Code (BASIC)"
                                                          action:@selector(renumberScript)
                                                   keyEquivalent:@"R"];
    [renumberItem setKeyEquivalentModifierMask:NSEventModifierFlagCommand | NSEventModifierFlagShift];
    [renumberItem setTarget:self];
    [editMenu addItem:renumberItem];

    NSMenuItem* removeNumbersItem = [[NSMenuItem alloc] initWithTitle:@"Remove Line Numbers (BASIC)"
                                                               action:@selector(removeLineNumbers)
                                                        keyEquivalent:@""];
    [removeNumbersItem setTarget:self];
    [editMenu addItem:removeNumbersItem];
    ...
    NSMenuItem* runItem = [[NSMenuItem alloc] initWithTitle:@"Run Script"
...
- (BOOL)initializeFrameworkWithConfig:(DisplayConfig)config {
    ...
    if (!Logger::instance().initialize(logConfig)) {
        NSLog(@"[BaseRunner] ERROR: Failed to initialize logger");
...
- (void)applicationDidFinishLaunching:(NSNotification*)notification {
    NSLog(@"[BaseRunner] %s starting...", self.runnerName.c_str());

    // Setup menu bar first
    NSLog(@"[BaseRunner] Setting up menu bar...");
    [self setupMenuBar];
    ...
    NSLog(@"[BaseRunner] Marking DisplayManager ready for rendering...");
    _displayManager->setReady();
    NSLog(@"[BaseRunner] ✓ DisplayManager ready");
    ...
    if (!self.headless) {
        ...
        NSLog(@"[BaseRunner] Headless mode - window not shown");
    }
    ...
    if (!_startupStateMachine->transitionTo(StartupState::STARTING_RENDER_LOOP, "Starting render loop")) {
        NSLog(@"[BaseRunner] ERROR: Invalid state transition to STARTING_RENDER_LOOP");
        [NSApp terminate:nil];
        return;
    }
    ...
    NSLog(@"[BaseRunner] %s initialized successfully - STATE: RUNNING", self.runnerName.c_str());
}

- (void)applicationWillTerminate:(NSNotification*)notification {
    NSLog(@"[BaseRunner] %s shutting down...", self.runnerName.c_str());
    ...
    // Stop display link
    if (_displayLink) {
        NSLog(@"[BaseRunner] Stopping display link...");
    ...
    if (_scriptThread.joinable()) {
        NSLog(@"[BaseRunner] Waiting for script thread to finish...");
        _scriptThread.join();
        NSLog(@"[BaseRunner] Script thread joined");
    }

    // Clean up language-specific runtime
    NSLog(@"[BaseRunner] Cleaning up runtime...");
    [self cleanupRuntime];

    NSLog(@"[BaseRunner] %s shutdown complete", self.runnerName.c_str());
}
...
static CVReturn DisplayLinkCallback(CVDisplayLinkRef displayLink,
    ...
    @autoreleasepool {
        BaseRunner* self = (__bridge BaseRunner*)displayLinkContext;
        [self onFrameTick];
    }
...
- (void)onFrameTick {
    ...
    if (!_running && _scriptThread.joinable()) {
        ...
        NSLog(@"[BaseRunner] Script finished, shutting down...");
        _scriptThread.join();
...
- (BOOL)initializeEditor {
    NSLog(@"[BaseRunner] Initializing editor subsystem...");
    ...
    NSLog(@"[BaseRunner] ✓ Screen state manager created");
    ...
    NSLog(@"[BaseRunner] ✓ Text editor initialized");

    // Register editor instance with C API
    editor_set_instance(_textEditor.get());
    NSLog(@"[BaseRunner] ✓ Editor instance registered with C API");
    ...
    NSLog(@"[BaseRunner] ✓ Editor subsystem initialized (using DisplayManager status bar)");
    return YES;
}
...
- (void)updateEditor:(double)deltaTime {
    ...
    if (_displayManager) {
        ...
        if (logCount < 10) {
            NSLog(@"[BaseRunner] Frame %d - Filename from editor: '%s' (length: %zu)",
        ...
        if (!filename.empty()) {
            for (char c : filename) {
                if (c < 32 || c > 126) {  // Not printable ASCII
                    filenameValid = false;
                    NSLog(@"[BaseRunner] WARNING: Invalid character in filename (code: %d)", (int)c);
                    break;
                }
            }
        }
        ...
        if (!filenameValid || filename.empty()) {
            filename = "untitled";
            if (!filenameValid) {
                NSLog(@"[BaseRunner] WARNING: Filename contained non-printable characters, reset to 'untitled'");
            }
        }
...
- (void)enterEditorMode {
    ...
    // This must happen AFTER mode switching to ensure the window is ready
    if (_displayManager) {
        NSView* displayView = _displayManager->getMetalView();
        if (displayView && self.window) {
            [self.window makeFirstResponder:displayView];
            NSLog(@"[BaseRunner] Made display view first responder for editor mode");
        }
    }
}
...
- (void)enterEditorModeAndClear {
    ...
    NSLog(@"[BaseRunner] === enterEditorModeAndClear ===");
    ...
    if (_screenStateManager) {
        ...
        NSLog(@"[BaseRunner]   Invalidated editor state");
    }
    ...
    if (_textGrid) {
        _textGrid->clear();
        NSLog(@"[BaseRunner]   Cleared text grid");
    }
    ...
    if (_textEditor) {
        _textEditor->render();
        NSLog(@"[BaseRunner]   Rendered editor content");
    }
    ...
    if (_displayManager) {
        ...
        if (displayView && self.window) {
            [self.window makeFirstResponder:displayView];
            NSLog(@"[BaseRunner]   Made display view first responder");
        }
    }

    NSLog(@"[BaseRunner] ✓ Editor mode entered with clear state");
}

- (void)enterRuntimeMode {
    NSLog(@"[BaseRunner] Entering runtime mode");
    ...
    // Clear particle system state for fresh script run
    st_particle_clear();
    ...
    NSLog(@"[BaseRunner] ✓ Runtime mode active (screen cleared)");
}
...
- (void)changeWindowSizeWithWidth:(uint32_t)width height:(uint32_t)height cellWidth:(uint32_t)cellW cellHeight:(uint32_t)cellH {
    NSLog(@"[BaseRunner] Resizing window to %dx%d (cells: %dx%d)", width, height, cellW, cellH);

    // Save current state based on mode
    if (_screenStateManager) {
        if (self.editorMode) {
            _screenStateManager->saveEditorState();
            NSLog(@"[BaseRunner] Saved editor state");
        } else {
            _screenStateManager->saveRuntimeState();
            NSLog(@"[BaseRunner] Saved runtime state");
        }
    }
    ...
    NSLog(@"[BaseRunner] New text grid: %dx%d", newTextCols, newTextRows);
    ...
    NSLog(@"[BaseRunner] TextGrid recreated (%dx%d)", newTextCols, newTextRows);

    // Recreate graphics layer
    _graphicsLayer = std::make_shared<GraphicsLayer>();
    NSLog(@"[BaseRunner] GraphicsLayer recreated");

    // Reload FontAtlas at new size
    NSLog(@"[BaseRunner] Reloading FontAtlas for cell size %dx%d", cellW, cellH);
    _fontAtlas = std::make_shared<FontAtlas>(_renderer->getDevice());
    ...
    if (_fontAtlas->loadUnsciiFont(unsciiVariant, fontSize)) {
        NSLog(@"[BaseRunner] FontAtlas reloaded (Unscii %s %dpt)", unsciiVariant.c_str(), fontSize);
    } else {
        // Fallback to system fonts
        NSLog(@"[BaseRunner] Unscii not available, using system fonts...");
        std::string fontPath = "/System/Library/Fonts/Menlo.ttc";
        if (!_fontAtlas->loadTrueTypeFont(fontPath, fontSize)) {
            ...
            if (!_fontAtlas->loadTrueTypeFont(fontPath, fontSize)) {
                ...
                if (!_fontAtlas->loadBuiltinFont(builtinFont)) {
                    NSLog(@"[BaseRunner] WARNING: Failed to reload font at size %d", fontSize);
                } else {
                    NSLog(@"[BaseRunner] FontAtlas reloaded (builtin VGA)");
                }
            } else {
                NSLog(@"[BaseRunner] FontAtlas reloaded (Monaco %dpt)", fontSize);
            }
        } else {
            NSLog(@"[BaseRunner] FontAtlas reloaded (Menlo %dpt)", fontSize);
        }
    }
    ...
    if (_inputManager && _textGrid && _displayManager) {
        ...
        if (metalView) {
            ...
            NSLog(@"[BaseRunner] InputManager cell size updated to %.2fx%.2f (actual view: %.0fx%.0f, grid: %dx%d)",
                  actualCellW, actualCellH, actualWidth, actualHeight, newTextCols, newTextRows);
            NSLog(@"[BaseRunner] This ensures mouse coordinates (in view points) correctly map to grid cells");
        } else {
            NSLog(@"[BaseRunner] WARNING: Could not get metalView for cell size calculation");
        }
    }
    ...
    if (_textEditor) {
        NSLog(@"[BaseRunner] Updating editor with new text grid");
        _textEditor->setTextGrid(_textGrid);
        NSLog(@"[BaseRunner] Editor updated and re-rendered");
    }

    // Restore state for current mode
    if (self.editorMode) {
        NSLog(@"[BaseRunner] In editor mode - editor should be visible");
        // Editor already updated and rendered above
    } else {
        NSLog(@"[BaseRunner] In runtime mode");
    ...
    NSLog(@"[BaseRunner] ✓ Window resize complete");
}
...
- (void)newScript {
    NSLog(@"[BaseRunner] Creating new script (default)");
    [self newBasicScript];  // Default to BASIC
}

- (void)newBasicScript {
    NSLog(@"[BaseRunner] === Creating new BASIC script (DOCUMENT-BASED) ===");
    ...
    NSLog(@"[BaseRunner] ✓ New BASIC document created and connected");
...
- (void)newAbcScript {
    NSLog(@"[BaseRunner] === Creating new ABC script (DOCUMENT-BASED) ===");
    ...
    NSLog(@"[BaseRunner] ✓ New ABC document created and connected");
...
- (void)newVoiceScript {
    NSLog(@"[BaseRunner] === Creating new VoiceScript (DOCUMENT-BASED) ===");
    ...
    NSLog(@"[BaseRunner] ✓ New VoiceScript document created and connected");
...
- (void)openScript {
    ...
    if (![NSThread isMainThread]) {
        NSLog(@"[BaseRunner] openScript called from background thread, dispatching to main");
        dispatch_async(dispatch_get_main_queue(), ^{
            [self openScript];
        });
        return;
    ...
    NSLog(@"[BaseRunner] === OPEN SCRIPT (DOCUMENT-BASED) ===");

    if (!_scriptDatabase || !_textEditor) {
        NSLog(@"[BaseRunner] ERROR: Missing components - database=%p editor=%p",
    ...
    // Close any open document without prompting
    if (_currentDocument && _currentDocument->isOpen()) {
        NSLog(@"[BaseRunner] Closing existing document: '%s'", _currentDocument->getName().c_str());
        _currentDocument->close();
    }
    ...
    if (_screenStateManager) {
        _screenStateManager->invalidateEditorState();
        NSLog(@"[BaseRunner] Cleared saved editor state before opening");
    }
    ...
    if (!dialogResult || selectedName.empty()) {
        NSLog(@"[BaseRunner] User cancelled or invalid selection");
        return;
    }

    NSLog(@"[BaseRunner] Selected: '%s' (language: %d, from cart: %d)",
          selectedName.c_str(), (int)selectedLanguage, isFromCart);

    // =========================================================================
    // STEP 1: Create/Open Document (SINGLE SOURCE OF TRUTH)
    // =========================================================================
    NSLog(@"[BaseRunner] === Creating Document instance ===");

    // Create new document if needed
    if (!_currentDocument) {
        _currentDocument = std::make_shared<Document>();
        NSLog(@"[BaseRunner]   Created new Document instance");
    }

    // Open the document from database or cart
    if (isFromCart) {
        // Load from cart data file
        NSLog(@"[BaseRunner] Loading script from cart: '%s'", selectedName.c_str());

        if (!_cartManager || !_cartManager->isCartActive()) {
            NSLog(@"[BaseRunner] ERROR: Cart not active");
        ...
        if (!cartLoader) {
            NSLog(@"[BaseRunner] ERROR: No cart loader");
        ...
        if (!cartLoader->loadDataFile(selectedName, dataFile)) {
            NSLog(@"[BaseRunner] ERROR: Failed to load cart file: %s",
        ...
        NSLog(@"[BaseRunner] ✓ Loaded %zu bytes from cart", content.length());
    } else {
        // Load from database
        if (!_currentDocument->open(_scriptDatabase, selectedName, selectedLanguage)) {
            NSLog(@"[BaseRunner] ERROR: Failed to open document: %s",
    ...
    NSLog(@"[BaseRunner] ✓ Document opened successfully");
    NSLog(@"[BaseRunner]   Name: '%s'", _currentDocument->getName().c_str());
    NSLog(@"[BaseRunner]   Lines: %zu", _currentDocument->getLineCount());
    NSLog(@"[BaseRunner]   Language: %d", (int)_currentDocument->getLanguage());
    ...
    // =========================================================================
    if (selectedLanguage == ScriptLanguage::BASIC) {
        ...
        if (FasterBASIC::hasValidLineNumbers(content)) {
            ...
            if (result.success) {
                _currentDocument->setText(result.formatted_code);
                _currentDocument->save();  // Save formatted version
                NSLog(@"[BaseRunner] ✓ Auto-formatted %d BASIC lines", result.lines_processed);
            }
        }
    }
    ...
    // =========================================================================
    NSLog(@"[BaseRunner] === Connecting Document to TextEditor ===");

    // Set filename
    _textEditor->setFilename(selectedName);
    NSLog(@"[BaseRunner]   Set filename: '%s'", selectedName.c_str());
    ...
    NSLog(@"[BaseRunner]   Document connected: %zu chars, %zu lines",
          _currentDocument->getText().length(), _currentDocument->getLineCount());
    NSLog(@"[BaseRunner]   Editor buffer now has: %zu lines",
    ...
    NSLog(@"[BaseRunner] ✓ Script opened: '%s' (%zu lines)",
          selectedName.c_str(), _currentDocument->getLineCount());
    NSLog(@"[BaseRunner] === OPEN COMPLETE (Document is now single source of truth) ===");
}
...
- (void)saveScript {
    ...
    NSLog(@"[BaseRunner] === SAVE SCRIPT (DOCUMENT-BASED) ===");

    if (!_currentDocument || !_currentDocument->isOpen()) {
        NSLog(@"[BaseRunner] No document open");
        return;
    }
    ...
    if (currentFilename.empty() || currentFilename.find("untitled") == 0) {
        NSLog(@"[BaseRunner] File is new/untitled, redirecting to Save As");
        [self saveScriptAs];
        return;
    }

    NSLog(@"[BaseRunner] Saving document: '%s'", currentFilename.c_str());
    ...
    if (_currentDocument->save()) {
        _textEditor->markClean();
        NSLog(@"[BaseRunner] ✓ Document saved successfully: '%s'", currentFilename.c_str());

        // Update status bar
        [self updateEditor:0.0];
    } else {
        NSLog(@"[BaseRunner] ERROR: Failed to save document: %s",
              _currentDocument->getLastError().c_str());
...
- (void)saveScriptAs {
    ...
    NSLog(@"[BaseRunner] === Save script as... (DOCUMENT-BASED) ===");

    if (!_currentDocument || !_currentDocument->isOpen() || !_textEditor) {
        NSLog(@"[BaseRunner] ERROR: No document open or missing editor");
        return;
    }

    std::string currentFilename = _currentDocument->getName();
    NSLog(@"[BaseRunner] Current document name: '%s'", currentFilename.c_str());
    ...
    [alert beginSheetModalForWindow:self.window completionHandler:^(NSModalResponse returnCode) {
        if (returnCode == NSAlertFirstButtonReturn) {
            ...
            if (name.length == 0) {
                NSLog(@"[BaseRunner] Save cancelled - no name entered");
                return;
            }

            std::string newName = std::string([name UTF8String]);
            NSLog(@"[BaseRunner] User entered name: '%s'", newName.c_str());
            ...
            if (currentLang == ScriptLanguage::ABC && newName.find(".abc") == std::string::npos) {
                newName += ".abc";
                NSLog(@"[BaseRunner] Auto-added .abc extension: '%s'", newName.c_str());
                ...
                newName += ".vscript";
                NSLog(@"[BaseRunner] Auto-added .vscript extension: '%s'", newName.c_str());
            }

            // Use Document's saveAs method (maintains single source of truth)
            NSLog(@"[BaseRunner] Calling Document::saveAs with new name: '%s'", newName.c_str());
            if (_currentDocument->saveAs(newName)) {
                _textEditor->markClean();
                NSLog(@"[BaseRunner] ✓ Script saved as: %s", newName.c_str());
                NSLog(@"[BaseRunner]   Document name: '%s'", _currentDocument->getName().c_str());
                NSLog(@"[BaseRunner]   Language: %d", (int)_currentDocument->getLanguage());
...
- (void)removeLineNumbers {
        ...
        NSLog(@"[BaseRunner] Removing line numbers from script");

        if (!_textEditor) {
            NSLog(@"[BaseRunner] ERROR: No text editor available");
            return;
        }
        ...
        if (source_code.empty()) {
            NSLog(@"[BaseRunner] No code to process");
            return;
        }

        // Remove line numbers using the formatter
        FasterBASIC::FormatterResult result = FasterBASIC::removeLineNumbers(source_code);

        if (result.success) {
            // Replace editor content with unnumbered code
            _textEditor->loadText(result.formatted_code);
            _textEditor->markDirty(); // Mark as modified so user can save

            NSLog(@"[BaseRunner] ✓ Removed line numbers from %d lines", result.lines_processed);

            // Show success message
            NSString* message = [NSString stringWithFormat:@"Removed line numbers from %d lines",
                                result.lines_processed];

            NSAlert* alert = [[NSAlert alloc] init];
            [alert setMessageText:@"Remove Line Numbers Complete"];
            [alert setInformativeText:message];
            [alert addButtonWithTitle:@"OK"];
            [alert beginSheetModalForWindow:self.window completionHandler:nil];

        } else {
            NSLog(@"[BaseRunner] ERROR: Remove line numbers failed: %s", result.error_message.c_str());

            NSString* errorMsg = [NSString stringWithUTF8String:result.error_message.c_str()];
            NSAlert* alert = [[NSAlert alloc] init];
            [alert setMessageText:@"Remove Line Numbers Failed"];
...
- (void)formatScriptWithStartLine:(int)startLine step:(int)step {
    ...
    NSLog(@"[BaseRunner] Formatting script (start: %d, step: %d)", startLine, step);
    ...
    if (self.currentScriptLanguage != "basic") {
        ...
        [alert setInformativeText:@"Code formatting is only available for BASIC scripts."];
        [alert addButtonWithTitle:@"OK"];
        [alert beginSheetModalForWindow:self.window completionHandler:nil];
        return;
    }
    ...
    if (source_code.empty()) {
        NSLog(@"[BaseRunner] No code to format");
        return;
    }
    ...
    FasterBASIC::FormatterResult result = FasterBASIC::formatBasicCode(source_code, options);

    if (result.success) {
        // Replace editor content with formatted code
        _textEditor->loadText(result.formatted_code);
        _textEditor->markDirty(); // Mark as modified so user can save

        NSLog(@"[BaseRunner] ✓ Formatted %d lines", result.lines_processed);

        // Show success message
        NSString* message = [NSString stringWithFormat:@"Formatted %d lines (line numbers %d to %d, step %d)",
                            result.lines_processed,
                            startLine,
                            startLine + (result.lines_processed - 1) * step,
                            step];
        ...
        NSLog(@"[BaseRunner] ERROR: Format failed: %s", result.error_message.c_str());

        NSString* errorMsg = [NSString stringWithUTF8String:result.error_message.c_str()];
        NSAlert* alert = [[NSAlert alloc] init];
        [alert setMessageText:@"Format Failed"];
        [alert setInformativeText:errorMsg];
        [alert addButtonWithTitle:@"OK"];
        [alert beginSheetModalForWindow:self.window completionHandler:nil];
    }
...
- (void)renumberScriptWithStartLine:(int)startLine step:(int)step {
    ...
    NSLog(@"[BaseRunner] Renumbering script (start: %d, step: %d)", startLine, step);
    ...
    if (source_code.empty()) {
        NSLog(@"[BaseRunner] No code to renumber");
        return;
    }

    // Renumber the code (use renumberBasicCode for renumber-only operation)
    FasterBASIC::FormatterResult result = FasterBASIC::renumberBasicCode(source_code, startLine, step);

    if (result.success) {
        // Replace editor content with renumbered code
        _textEditor->loadText(result.formatted_code);
        _textEditor->markDirty(); // Mark as modified so user can save

        NSLog(@"[BaseRunner] ✓ Renumbered %d lines", result.lines_processed);

        // Show success message
        NSString* message = [NSString stringWithFormat:@"Renumbered %d lines (line numbers %d to %d, step %d)",
                            result.lines_processed,
                            startLine,
                            startLine + (result.lines_processed - 1) * step,
                            step];
        ...
        NSAlert* alert = [[NSAlert alloc] init];
        [alert setMessageText:@"Renumber Complete"];
        [alert setInformativeText:message];
        [alert addButtonWithTitle:@"OK"];
        [alert beginSheetModalForWindow:self.window completionHandler:nil];

    } else {
        NSLog(@"[BaseRunner] ERROR: Renumber failed: %s", result.error_message.c_str());

        NSString* errorMsg = [NSString stringWithUTF8String:result.error_message.c_str()];
        NSAlert* alert = [[NSAlert alloc] init];
        [alert setMessageText:@"Renumber Failed"];
        [alert setInformativeText:errorMsg];
        [alert addButtonWithTitle:@"OK"];
        [alert beginSheetModalForWindow:self.window completionHandler:nil];
    }
...
- (void)closeScript {
    NSLog(@"[BaseRunner] Closing script");

    if (!_textEditor) return;

    // Don't prompt for unsaved changes - user is responsible for saving
    NSLog(@"[BaseRunner] Closing script (no unsaved changes check)");

    [self enterRuntimeMode];
}

- (void)playCurrentFile {
    NSLog(@"[BaseRunner] === Playing current file ===");

    if (!_currentDocument || !_currentDocument->isOpen()) {
        NSLog(@"[BaseRunner] ERROR: No document open");
        return;
    }
    ...
    NSLog(@"[BaseRunner] Document: '%s', Language: %d", filename.c_str(), (int)language);

    // Handle ABC music files
    if (language == ScriptLanguage::ABC) {
        NSLog(@"[BaseRunner] ABC file detected, playing music...");
        ...
        NSLog(@"[BaseRunner] ABC music playing");
        return;
    }

    // Handle VoiceScript files
    if (language == ScriptLanguage::VOICESCRIPT) {
        NSLog(@"[BaseRunner] VoiceScript file detected, playing audio...");
        ...
        if (_audioManager) {
            ...
            if (_audioManager->voiceScriptDefine(scriptName, content, error)) {
                ...
                if (_audioManager->voiceScriptPlay(scriptName, 120.0f)) {
                    ...
                    NSLog(@"[BaseRunner] VoiceScript audio playing");
                } else {
                    NSLog(@"[BaseRunner] ERROR: Failed to play VoiceScript");
                    [self showError:@"Failed to play VoiceScript"];
                }
            } else {
                NSLog(@"[BaseRunner] ERROR: Failed to compile VoiceScript: %s", error.c_str());
                [self showError:[NSString stringWithFormat:@"Failed to compile VoiceScript:\n%s", error.c_str()]];
            }
        } else {
            NSLog(@"[BaseRunner] ERROR: AudioManager not available");
    ...
    // Not a music file
    NSLog(@"[BaseRunner] File is not a playable music file (language: %d)", (int)language);
    [self showError:@"Current file is not a playable music file.\nSupported formats: ABC, VoiceScript"];
}

- (void)runScript {
    NSLog(@"[BaseRunner] === Running script (DOCUMENT-BASED) ===");
    ...
    NSLog(@"[BaseRunner] Entering runtime mode for language: %d", (int)language);
    [self enterRuntimeMode];
}

- (void)stopScript {
    NSLog(@"[BaseRunner] Stopping script");

    // Clear all text display items to prevent stale GPU resource access
    NSLog(@"[BaseRunner] Clearing text display items...");
    st_text_clear_displayed();
    NSLog(@"[BaseRunner] Text display items cleared");

    // Reset display mode to text mode (mode 0)
    NSLog(@"[BaseRunner] Resetting display mode to text mode...");
    st_mode(0);
    NSLog(@"[BaseRunner] Display mode reset complete");

    // TODO: Stop script execution
}

- (void)exportContent {
    NSLog(@"[BaseRunner] Export content");
...
- (void)importContent {
    NSLog(@"[BaseRunner] Import content");
...
- (void)clearOutput {
    NSLog(@"[BaseRunner] Clearing output");
...
- (void)findNext:(id)sender {
    ...
    NSLog(@"[BaseRunner] Find next - not yet implemented (need to track last search)");
}
...
- (void)showGotoLine:(id)sender {
    ...
    if ([GotoLineDialog showDialogWithCurrentLine:currentLine
        ...
        if (targetLine > 0 && targetLine <= maxLine) {
            ...
            NSLog(@"[BaseRunner] Jumped to line %ld", (long)targetLine);
        }
    }
}
...
- (void)performFind:(NSString*)findText options:(FindOptions)options {
    ...
    if (result.found) {
        ...
        NSLog(@"[BaseRunner] Found at line %zu, column %zu", result.line + 1, result.column + 1);
    } else {
        NSBeep();
        NSLog(@"[BaseRunner] Not found: %@", findText);
    }
}
        ...
        case FindReplaceActionReplaceAll: {
            int count = TextSearcher::replaceAll(*buffer,
                                                *cursor,
                                                ...
                                                caseSensitive,
                                                wholeWord);
            NSLog(@"[BaseRunner] Replaced %d occurrence(s)", count);
            if (count == 0) {
                NSBeep();
            }
            break;
